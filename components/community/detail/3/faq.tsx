...
export default function Faq({ clsFilter }: IProps) {
  const faqList = [
    {
      id: 0,
      title: '결제 후 모임에 참여하면 시작은 어떻게 하면 될까요?',
      content:
        '이 스터디 모임은 2주에 한 권씩 정해진 순서대로 양서를 읽음과 동시에 책에서 배운 내용을 삶에 적용해 스스로 원하는 도전 목표를 세우고 실행해 나가는 방식으로 운영됩니다. 실행에 대한 챌린지는 리더의 안내에 따라 과제를 수행해 나가는 방식입니다. 참여하면 안내에 따라 해당 책을 준비해 함께 읽기 시작하시고 출석체크와 실행 미션을 진행하시면 됩니다. 개인마다 챌린지 시작 및 종료 시점이 다르며 호스트가 개별 안내를 통해 활동을 가이드해 드립니다.',
    },
    {
      id: 1,
      title: '이미 읽은 책이 포함돼 있을 경우는 어떻게 하나요?',
      content:
        '이 모임은 책을 읽는 데 목적이 있는 게 아니라 삶에 직접 적용해 성장하는 데 있습니다. 같은 책도 어떻게 읽고 소화하느냐에 따라 배우고 활용하는 정도가 달라집니다. 기존에 읽은 책이 포함돼 있다 하더라도 가급적 함께 다시 한 번 읽고 미션을 수행하시는 것을 권장드립니다.',
    },
    {
      id: 2,
      title: '함께 읽는 책은 각자가 구매해야 하나요?',
      content:
        '네, 맞습니다. 종이책이든, e북이든 본인이 틈틈히 편하게 읽을 수 있는 방법을 찾아서 책을 확보하고 챌린지에 동참해주시면 됩니다. 국내에 한해 도서를 구하기 어려운 지역에 거주하고 계실 경우 고객센터로 문의주시면 호스트가 도서 확보를 도와드립니다.',
    },
    {
      id: 3,
